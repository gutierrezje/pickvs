...
1. Will users sign up for a platform to submit their predictions?
2. Can the system successfully ingest, process, and grade these predictions against a reliable, third-party data source?
3. Will a public leaderboard showing performance metrics (ROI, Net Units) drive user engagement?
...
- **Supported sports**: NBA only
- **Supported markets**: Moneyline, Point Spread, and Totals (Over/Under)

### 2.3. Out-of-Scope (Future Releases)

- All other sports (NFL, MLB, Soccer, etc.)
- Any paid API usage (system must operate entirely within The Odds API free tier)
- Player prop bets or any other complex markets
- Uploading model files (users submit picks, not proprietary model code)
- Real-time odds or live betting (all odds fetched pre-game, results processed post-game)
